...
// Dapatkan id tahun ajaran dengan prepared statement
$ta_id = 0;
if ($tahun_ajaran) {
...
if ($kelas_id > 0 && $ta_id > 0) {
    ...
    $stmt->bind_param("iiis", $kelas_id, $kelas_id, $ta_id, $semester);
    $stmt->execute();
    $tugas_data = $stmt->get_result();
}

// Export Excel
if (isset($_GET['export_excel']) && $kelas_id > 0 && $tugas_data && $tugas_data->num_rows > 0) {
    header("Content-Type: application/vnd.ms-excel");
    header("Content-Disposition: attachment; filename=rekap_tugas_kelas_$kelas_id.xls");
    echo "<table border='1'>";
    echo "<tr><th>Judul</th><th>Mapel</th><th>Guru</th><th>Deadline</th><th>Kategori</th><th>Sudah Kumpul</th><th>Total Siswa</th></tr>";
    while($t = $tugas_data->fetch_assoc()){
        echo "<tr>
            ...
            <td>".htmlspecialchars($t['kategori_nilai'])."</td>
...
?>
<div class="page-header"><h2>Rekap Tugas per Kelas</h2></div>
<div class="form-container">
    <form method="GET" class="form-row">
        ...
        <div class="form-group"><label>Semester</label>
            <select name="semester" class="form-select"><?= semester_options($semester) ?></select>
        </div>
        <div class="form-group"><label>Tahun Ajaran</label>
            <select name="tahun" class="form-select"><?= tahun_ajaran_options($tahun_ajaran) ?></select>
        </div>
